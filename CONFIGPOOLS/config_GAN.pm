...
# output from which layer? A number is needed for each network in @sysdir
# 12 is the layer ID of the skipAdd layer in GAN network
# note: input layer has layer ID as 0
@tapLayer  = dupVec(12, scalar @sysdir);

# set it to false
@tapGate   = dupVec('false', scalar @sysdir); # output from highway gate (skipppara) ? 
...
@mdnGenPara  = dupVec(1, scalar @sysdir); 
# Because GAN has a MDN output, this configuration is necessary to make the
# generation process work
